...
//			container.children[ i ].style.display = i === id ? 'block' : 'none';
			container.children[ i ].style.display = i === 'inline-block';
		}
...
Stats.Panel = function ( name, fg, bg ) {
	...
	return {
		...
		update: function ( value, maxValue, minValue="" ) {
			...
			context.fillStyle = fg;
            if(name=="FPS"){ value=value.toFixed(2); }
            if(name=="MS"){ value=value.toFixed(0); }
            if(name=="MB"){ value=value.toFixed(0); }
            if(name=="GPU"){ value=value.toFixed(2); }
            //context.fillText( ( value ) + ' ' + name + ' (' + round( min ) + '-' + round( max ) + ')', TEXT_X, TEXT_Y );
            if(name=="GPU"){
            context.fillText( name+' '+minValue+' '+value, TEXT_X, TEXT_Y );
            }
            else{
            context.fillText( name+' '+value+minValue, TEXT_X, TEXT_Y );
            }
