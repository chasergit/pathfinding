...
const RAPIER_PATH = './rapier3d-compat.js';
...
async function RapierPhysics() {
	...
	if ( RAPIER === null ) {
		...
		console.warn = function (...args) {
			// Если варнинг содержит ложную фразу Rapier — просто молча его стираем
			if (args && args[0] && typeof args[0] === 'string' && args[0].includes("using deprecated parameters")) {
				return; 
			}
			// Все остальные реальные предупреждения движка выводим как обычно
			originalWarn.apply(console, args);
		};

		// 2. ВЫЗЫВАЕМ ВАШ КРИСТАЛЬНО ЧИСТЫЙ МЕТОД БЕЗ АРГУМЕНТОВ
		await RAPIER.init();

		// 3. МГНОВЕННО ВОЗВРАЩАЕМ ОРИГИНАЛЬНУЮ КОНСОЛЬ НА МЕСТО
