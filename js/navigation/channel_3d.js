...
ПО СТАНДАРТУ RECAST/DETOUR: В САМОМ КОНЦЕ ДЕЛАЕМ ОБЯЗАТЕЛЬНЫЙ ПРОХОД ВЫСОТНОЙ ПРИВЯЗКИ.
БЕРЁМ ПОЛУЧЕННУЮ ПЛОСКУЮ ТОЧКУ (X, Z), НАХОДИМ ТРЕУГОЛЬНИК NAVMESH, НАД КОТОРЫМ ЭТА ТОЧКА СЕЙЧАС ФИЗИЧЕСКИ НАХОДИТСЯ (ИСПОЛЬЗУЯ ЦЕПОЧКУ PATH), И ЛИНЕЙНО ИНТЕРПОЛИРУЕМ ВЫСОТУ Y ПО ПЛОСКОСТИ ЭТОГО ТРЕУГОЛЬНИКА.
...
// ДОПОЛНИТЕЛЬНЫЕ ПЛОСКИЕ КЭШ-БУФЕРЫ ДЛЯ ЧЕСТНОЙ 3D ИНТЕРПОЛЯЦИИ ВЫСОТЫ ПОРТАЛОВ ПО КАНОНУ RECAST
const portals_left_y=new Float64Array(max_length);
const portals_right_y=new Float64Array(max_length);
...
portals_left_y[0]=start_position.y;
portals_right_y[0]=start_position.y;
...
portals_left_y[portal_count]=v0.y;
portals_right_y[portal_count]=v1.y;
}
else{
portals_left[portal_count]=v1;
portals_right[portal_count]=v0;
portals_left_y[portal_count]=v1.y;
portals_right_y[portal_count]=v0.y;
}
portal_count++;
}


// ФИНАЛЬНЫЕ ВОРОТА ЦЕЛИ (НУЛЕВОЙ ШИРИНЫ)
portals_left[portal_count]=end_position;
portals_right[portal_count]=end_position;
portals_left_y[portal_count]=end_position.y;
portals_right_y[portal_count]=end_position.y;
portal_count++;
...
// СТАНДАРТ RECAST/DETOUR: ИНТЕРПОЛЯЦИЯ ВЫСОТЫ ПО ГЕОМЕТРИИ РЁБЕР


let resCount=0;


// СТАРТОВАЯ ТОЧКА ПУТИ ЗАПИСЫВАЕТСЯ В СВОЁМ ИСХОДНОМ ВИДЕ
path[resCount++]={x:start_position.x,y:start_position.y,z:start_position.z};


let currentPortalIdx=1; 


// ПРОХОДИМ ПО ВСЕМ ПЛОСКИМ ОТРЕЗКАМ
for(let j=0;j<smoothed_length-1;j++){


const segStart=smoothed[j];
const segEnd=smoothed[j+1];
const segStartX=segStart.x;
const segStartZ=segStart.z;
const segEndX=segEnd.x;
const segEndZ=segEnd.z;


// ВЕКТОР ТЕКУЩЕГО ПЛОСКОГО ОТРЕЗКА ПУТИ
const seg_dx=segEndX-segStartX;
const seg_dz=segEndZ-segStartZ;


// СКАНИРУЕМ ВСЕ ПОРТАЛЫ, ЛЕЖАЩИЕ ВНУТРИ ДИАПАЗОНА ЭТОГО ОТРЕЗКА
while (currentPortalIdx<portal_count-1){


const pLeft=portals_left[currentPortalIdx];
const pRight=portals_right[currentPortalIdx];


// Если дошли до ворот, являющихся концом отрезка, переходим к следующему сегменту
if(pLeft===segEnd || pRight===segEnd || (pLeft.x===segEndX && pLeft.z===segEndZ) || (pRight.x===segEndX && pRight.z===segEndZ)){
break;
}


const pleft_x=pLeft.x;
const pleft_z=pLeft.z;
const pright_x=pRight.x;
const pright_z=pRight.z;


// ВЕКТОР ТЕКУЩЕГО РЕБРА ПОРТАЛА
const edge_dx=pright_x-pleft_x;
const edge_dz=pright_z-pleft_z;


// РАСЧЁТ ПЕРЕСЕЧЕНИЯ ДВУХ ОТРЕЗКОВ В ПЛОСКОСТИ XZ
const denom=seg_dx*edge_dz-seg_dz*edge_dx;


if(Math.abs(denom)>EPSILON){


// u-ПОЛОЖЕНИЕ ПЕРЕСЕЧЕНИЯ НА ВЕКТОРЕ ПУТИ (ОТ 0 ДО 1)
const u=((pleft_x-segStartX)*edge_dz-(pleft_z-segStartZ)*edge_dx)/denom;


// t-ПОЛОЖЕНИЕ ТОЧКИ ПЕРЕСЕЧЕНИЯ НА САМОМ РЕБРЕ ПОРТАЛА (ОТ 0 ДО 1)
const t=((pleft_x-segStartX)*seg_dz-(pleft_z-segStartZ)*seg_dx)/denom;


// ЕСЛИ ОТРЕЗОК ПУТИ ФИЗИЧЕСКИ ПЕРЕСЁК СТВОРКИ ТЕКУЩЕГО ПОРТАЛА
if(u>=-EPSILON && u<=1.0+EPSILON && t>=-EPSILON && t<=1.0+EPSILON){


const interX=segStartX+u*seg_dx;
const interZ=segStartZ+u*seg_dz;


// СТАНДАРТ RECAST/DETOUR: ЛИНЕЙНО ИНТЕРПОЛИРУЕМ ВЫСОТУ Y СТРОГО ПО ГЕОМЕТРИИ РЕБРА ТЕКУЩЕГО ПОРТАЛА ЛАНДШАФТА НА ОСНОВЕ ЗАПЕЧЁННОГО t.
const leftY=portals_left_y[currentPortalIdx];
const rightY=portals_right_y[currentPortalIdx];


const clampedT=t<0?0:(t>1?1:t);
const interY=leftY+clampedT*(rightY-leftY);


// ЗАЩИТА ОТ ДУБЛИРОВАНИЯ ПУТЕВЫХ ТОЧЕК
const prevPoint=path[resCount-1];
if(Math.abs(prevPoint.x-interX)>EPSILON || Math.abs(prevPoint.z-interZ)>EPSILON){
path[resCount++]={x:interX,y:interY,z:interZ};
}
}
}
currentPortalIdx++;
...
// ФИКСИРУЕМ САМ ОПОРНЫЙ 3D-УГОЛ (APEX)
const prevPoint=path[resCount-1];
if(Math.abs(prevPoint.x-segEndX)>EPSILON || Math.abs(prevPoint.z-segEndZ)>EPSILON){
path[resCount++]={x:segEndX,y:segEnd.y,z:segEndZ};
}
}


// ФИНАЛЬНАЯ КОРРЕКТИРОВКА ДЛИНЫ МАССИВА РЕЗУЛЬТАТОВ
path.length=resCount;
}
}
